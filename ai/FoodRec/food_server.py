...
def process_features_for_api_recommendation(preprocessor, user_context_dict, all_restaurants_df_orig):
    if all_restaurants_df_orig.empty:
    ...
    try:
        ...
        print(
            f"API Server: Error during preproc transform for recommendation: {e}")
        return torch.empty(0), [], 0
...
def save_fl_global_model_to_disk():
    ...
    if fl_global_model_weights:
        try:
            ...
            print(
                f"FL Server: Global model saved to {config.GLOBAL_MODEL_SAVE_PATH}")
        except Exception as e:
            print(f"FL Server: Error saving global model: {e}")
    ...
    if os.path.exists(config.GLOBAL_MODEL_SAVE_PATH):
        try:
            loaded_data = torch.load(
                config.GLOBAL_MODEL_SAVE_PATH, map_location=torch.device('cpu'))
            if isinstance(loaded_data, OrderedDict) and all(isinstance(v, torch.Tensor) for v in loaded_data.values()):
                ...
                print(
                    f"FL Server: Global model loaded from {config.GLOBAL_MODEL_SAVE_PATH}")
                if 'fc1.weight' in fl_global_model_weights:
                    ...
                    if dim > 0:
                        ...
                        if api_input_dim == -1:
                            api_input_dim = dim  # Sync with API's global dim
                        print(
                            f"FL Server: INPUT_DIM set to {dim} from loaded model.")
            else:
                print(
                    f"FL Server: Loaded model from {config.GLOBAL_MODEL_SAVE_PATH} not valid state_dict.")
        except Exception as e:
            print(f"FL Server: Error loading model: {e}.")
    else:
        print(
            f"FL Server: No pre-saved FL model at {config.GLOBAL_MODEL_SAVE_PATH}.")
...
def get_fl_global_model_route():
    ...
    if fl_global_model_weights:
        try:
            model_dim_check = -1
            if 'fc1.weight' in fl_global_model_weights:
                model_dim_check = fl_global_model_weights['fc1.weight'].shape[1]
            if model_dim_check > 0 and current_dim_to_send > 0 and model_dim_check != current_dim_to_send:
                print(
                    f"FL Server WARNING in /get_global_model: model actual dim {model_dim_check} != reported dim {current_dim_to_send}. Reporting actual.")
            ...
            print(f"FL Server ERROR in /get_global_model serialization: {e}")
            traceback.print_exc()
...
def submit_fl_update_route():
    ...
    client_id, weights_s, size, c_in_dim = data.get('client_id'), data.get(
        'model_weights'), data.get('data_size'), data.get('input_dim', -1)
    ...
    try:
        ...
        if config.INPUT_DIM == -1 and c_in_dim > 0:
            ...
            if api_input_dim == -1:
                api_input_dim = c_in_dim
            print(
                f"FL Server: config.INPUT_DIM set to {config.INPUT_DIM} by first update from {client_id}.")
        ...
        fl_client_updates.append(
            {'weights': client_weights, 'size': size, 'client_id': client_id})
        ...
        if config.NUM_USERS > 0 and len(fl_client_updates) >= config.NUM_USERS:
            print(
                f"FL Server: Reached {len(fl_client_updates)} updates for FL round, aggregating...")
            # This will use and potentially set config.INPUT_DIM, api_input_dim
            aggregate_fl_updates()
            save_fl_global_model_to_disk()
            fl_client_updates.clear()
            fl_connected_clients_this_round.clear()
            fl_current_round_server += 1
            print(
                f"FL Server: Aggregation for FL round {fl_current_round_server} complete.")
        return jsonify({'status': 'success', 'message': 'FL Update received'})
    except Exception as e:
        print(f"FL Server: Error processing FL update from {client_id}: {e}")
        traceback.print_exc()
...
def aggregate_fl_updates():
    ...
    if not fl_client_updates:
        print("FL Server: No updates to aggregate.")
        return
    valid_updates = [
        upd for upd in fl_client_updates if upd['size'] > 0 and upd['weights']]
    if not valid_updates:
        print("FL Server: No valid FL updates.")
        return
    total_data_size = sum(upd['size'] for upd in valid_updates)
    if total_data_size == 0:
        print("FL Server: Total data size for FL is zero.")
        return
    ...
    if current_aggregation_input_dim <= 0:
        for upd_check_dim in valid_updates:
            if 'fc1.weight' in upd_check_dim['weights']:
                ...
                print(
                    f"FL Server: Deduced aggregation INPUT_DIM as {current_aggregation_input_dim} from first client update.")
                if api_input_dim == -1:
                    api_input_dim = current_aggregation_input_dim
                if config.INPUT_DIM == -1:
                    config.INPUT_DIM = current_aggregation_input_dim
                break
        if current_aggregation_input_dim <= 0:
            print(
                "FL Server: CRITICAL - INPUT_DIM unknown for aggregation. Cannot proceed.")
            return

    if fl_global_model_weights is None:
        try:
            print(
                f"FL Server: Initializing FL global model for aggregation with INPUT_DIM: {current_aggregation_input_dim}")
            model_template = FoodPreferenceModel(
                input_dim=current_aggregation_input_dim)
            fl_global_model_weights = OrderedDict()
            for k, v_template in model_template.state_dict().items():
                if v_template.is_floating_point():
                    fl_global_model_weights[k] = torch.zeros_like(
                        v_template, dtype=torch.float32, device='cpu')
            if not fl_global_model_weights:
                print("FL Server: ERROR - No float params in model template.")
                return
        except Exception as e_agg_init:
            print(
                f"FL Server: ERROR creating model template (dim {current_aggregation_input_dim}): {e_agg_init}")
            return

    for key_g in fl_global_model_weights.keys():
        if fl_global_model_weights[key_g].is_floating_point():
            fl_global_model_weights[key_g].zero_()
    ...
    for upd_agg in valid_updates:
        ...
        for key_g_check in fl_global_model_weights.keys():
            if key_g_check not in client_w:
                compatible = False
                print(
                    f"FL Server: Key '{key_g_check}' missing from {client_id_str}.")
                break
            if not client_w[key_g_check].is_floating_point():
                continue
            if fl_global_model_weights[key_g_check].shape != client_w[key_g_check].shape:
                compatible = False
                print(
                    f"FL Server: Shape mismatch for '{key_g_check}' from {client_id_str}. Global {fl_global_model_weights[key_g_check].shape}, Client {client_w[key_g_check].shape}.")
                break
        if not compatible:
            continue
        for key_g_agg in fl_global_model_weights.keys():
            if key_g_agg in client_w and client_w[key_g_agg].is_floating_point():
                try:
                    fl_global_model_weights[key_g_agg] += client_w[key_g_agg].to(
                        fl_global_model_weights[key_g_agg].device, dtype=torch.float32) * factor
                except RuntimeError as e_rt:
                    print(
                        f"FL Server: Aggregation RuntimeError for '{key_g_agg}' from {client_id_str}: {e_rt}")
        num_aggregated_clients += 1
    if num_aggregated_clients > 0:
        print(
            f"FL Server: FL Global model updated from {num_aggregated_clients} clients.")
    else:
        print("FL Server: No client updates were compatible for FL aggregation this round.")
...
def recommend_route():
    ...
    all_user_side_features_expected = [col for col in config.CATEGORY_COLS +
                                       config.NUMERIC_COLS if col not in api_all_restaurants_df.columns]
    ...
    try:
        ...
        return jsonify({"error": f"Feature processing error: {str(e)}"}), 500
    if features_tensor.nelement() == 0:
        return jsonify({"message": "No features processed.", "recommendations": []})
    if num_feat_proc != api_input_dim:
        return jsonify({"error": f"Feature mismatch! Model expects {api_input_dim}, Preproc: {num_feat_proc}."}), 500
    reco_inference_model.eval()
    with torch.no_grad():
        logits = reco_inference_model(features_tensor)
        probs = torch.softmax(logits, dim=1)
        high_pref_scores = probs[:, 2]
        if high_pref_scores.numel() == 0:
            return jsonify({"message": "No scores generated.", "recommendations": []})
        k = min(20, high_pref_scores.numel(), len(restaurant_ids_order))
        if k == 0:
            return jsonify({"message": "Not enough data for recs.", "recommendations": []})
        top_scores_t, top_indices_t = torch.topk(high_pref_scores, k=k)
        rec_ids = [restaurant_ids_order[i] for i in top_indices_t.tolist()]
        top_recs_df = api_all_restaurants_df[api_all_restaurants_df['restaurant_id'].isin(
            rec_ids)].copy()
        id_score_map_inf = dict(zip(rec_ids, top_scores_t.tolist()))
        top_recs_df['recommendation_score'] = top_recs_df['restaurant_id'].map(
            id_score_map_inf)
        recs_output = top_recs_df.sort_values(
            by='recommendation_score', ascending=False).fillna('N/A').to_dict(orient='records')
...
def train_user_model_route():
    ...
    global api_all_restaurants_df, api_preprocessor, api_input_dim, config
    ...
    if 'application/json' in content_type:
        print("Training API: Received JSON payload for user reviews.")
        data = request.get_json()
        print(f"Training API: JSON data received: {data}")
        user_reviews_csv_string = data.get('user_reviews_csv')
        if not user_reviews_csv_string:
            return jsonify({"error": "Missing 'user_reviews_csv'."}), 400
    ...
    if client_review_data.empty:
        return jsonify({"error": "User review data empty/invalid."}), 400
    ...
    profile_cols = [
        col for col in config.USER_PROFILE_COLS if col in client_review_data.columns]
    ...
    try:
        ...
        print(
            f"Training API ERROR: Client {client_id_str} init failed: {e_init}")
        traceback.print_exc()
    ...
    if not client_instance.model or client_instance.input_dim <= 0:
        ...
        if api_input_dim > 0 and (not client_instance.model or client_instance.input_dim <= 0):
            print(
                f"Training API Info: Client {client_id_str} model error, but global api_input_dim ({api_input_dim}) is set. Trying to re-init client model.")
            try:
                client_instance.input_dim = api_input_dim
                client_instance.model = FoodPreferenceModel(
                    input_dim=api_input_dim)
                print(
                    f"Training API Info: Re-initialized client {client_id_str} model with input_dim {api_input_dim}")
            except Exception as e_reinit:
                print(
                    f"Training API Error: Failed to re-init client {client_id_str} model: {e_reinit}")
    ...
    # Uses Client's internal LOCAL_EPOCHS or API_TRAIN_EPOCHS if Client is adapted
    avg_loss = client_instance.train_local_model()

    update_sent_status_message = "NotAttempted"
    if config.API_SEND_UPDATE_TO_SERVER:
        print(
            f"Training API: Client {client_id_str} attempting to send update via HTTP POST.")
...
def _infer_input_dim_from_state_dict(state_dict):
    if state_dict and 'fc1.weight' in state_dict:
        return state_dict['fc1.weight'].shape[1]
    return -1
...
def load_all_global_resources():
    ...
    # Populates fl_global_model_weights, can set config.INPUT_DIM, api_input_dim
    load_fl_global_model_from_disk()

    _, restaurants_df_from_generator = get_shanghai_data_for_simulation()
    if restaurants_df_from_generator.empty:
        print("CRITICAL: No Shanghai restaurant data for API. Exiting.")
        exit(1)
    api_all_restaurants_df = restaurants_df_from_generator
    if 'id' in api_all_restaurants_df.columns:
        api_all_restaurants_df.rename(
            columns={'id': 'restaurant_id'}, inplace=True)
    if 'rating' in api_all_restaurants_df.columns:
        api_all_restaurants_df.rename(
            columns={'rating': 'rating_biz'}, inplace=True)
    print(
        f"API Server: Loaded {len(api_all_restaurants_df)} Shanghai restaurants.")
    ...
    if os.path.exists(config.PREPROCESSOR_SAVE_PATH):
        try:
            ...
            print(
                f"API Server: Loaded shared preprocessor from {config.PREPROCESSOR_SAVE_PATH} for API and Client class.")
            if api_input_dim == -1 and hasattr(api_preprocessor, 'transformers_'):
                ...
                dummy_df_for_shape = pd.DataFrame(
                    temp_data, columns=dummy_df_cols)
                try:
                    transformed_dummy = api_preprocessor.transform(
                        dummy_df_for_shape[config.CATEGORY_COLS + config.NUMERIC_COLS])
                    inferred_dim = transformed_dummy.shape[1]
                    if inferred_dim > 0:
                        api_input_dim = inferred_dim
                        if config.INPUT_DIM == -1:
                            config.INPUT_DIM = inferred_dim
                        print(
                            f"API Server: Inferred INPUT_DIM={api_input_dim} from loaded preprocessor.")
                except Exception as e_trans_dummy:
                    print(
                        f"API Server: Could not infer INPUT_DIM from preprocessor: {e_trans_dummy}")
        except Exception as e_load_p:
            print(
                f"API Server: Preprocessor load FAILED: {e_load_p}. CRITICAL: Preprocessor must be pre-trained.")
            exit(1)
    else:
        print(
            f"API Server: CRITICAL - Preprocessor not found at {config.PREPROCESSOR_SAVE_PATH}. Must be pre-trained.")
        exit(1)

    if not os.path.exists(config.GLOBAL_MODEL_SAVE_PATH):
        print(
            f"API Server WARNING: Recommendation model {config.GLOBAL_MODEL_SAVE_PATH} not found. /recommend might fail until FL model is trained.")
        reco_inference_model = None  # No static model for reco
    else:
        try:
            reco_model_state_dict = torch.load(
                config.GLOBAL_MODEL_SAVE_PATH, map_location=torch.device('cpu'))
            dim_from_reco_model = _infer_input_dim_from_state_dict(
                reco_model_state_dict)
            if api_input_dim == -1 and dim_from_reco_model > 0:
                api_input_dim = dim_from_reco_model
                if config.INPUT_DIM == -1:
                    config.INPUT_DIM = api_input_dim
                print(
                    f"API Server: INPUT_DIM set to {api_input_dim} from recommendation model.")
            elif api_input_dim > 0 and dim_from_reco_model > 0 and api_input_dim != dim_from_reco_model:
                print(
                    f"API Server WARNING: api_input_dim ({api_input_dim}) differs from reco model's dim ({dim_from_reco_model}). Using {api_input_dim}.")
            if api_input_dim <= 0:
                print(
                    f"API Server CRITICAL: Final INPUT_DIM for reco model is {api_input_dim}. Exiting.")
                exit(1)
            reco_inference_model = FoodPreferenceModel(input_dim=api_input_dim)
            reco_inference_model.load_state_dict(
                reco_model_state_dict, strict=False)
            reco_inference_model.eval()
            print(
                f"API Server: Recommendation model weights loaded (INPUT_DIM: {api_input_dim}).")
        except Exception as e:
            print(
                f"API Server WARNING: Error loading recommendation model: {e}. /recommend may fail.")
            reco_inference_model = None
    ...
    if api_input_dim > 0:
        if config.INPUT_DIM != api_input_dim:
            print(
                f"API Server: Synchronizing config.INPUT_DIM ({config.INPUT_DIM}) to api_input_dim ({api_input_dim}).")
            config.INPUT_DIM = api_input_dim
    elif config.INPUT_DIM > 0:  # api_input_dim was not set, use config.INPUT_DIM
        api_input_dim = config.INPUT_DIM
        print(
            f"API Server: Setting api_input_dim from config.INPUT_DIM to {api_input_dim}.")
    else:  # Still no valid dimension
        print("API Server CRITICAL: Could not determine a valid INPUT_DIM. Exiting.")
        exit(1)

    print(
        f"--- Combined Server: All global resources loaded. Final effective INPUT_DIM: {api_input_dim} ---")


if __name__ == '__main__':
    if not hasattr(config, 'USER_PROFILE_COLS'):
        print("CRITICAL: 'USER_PROFILE_COLS' not defined in config.")
        exit(1)
    if not hasattr(config, 'API_TRAIN_EPOCHS'):
        config.API_TRAIN_EPOCHS = 1
    if not hasattr(config, 'API_SEND_UPDATE_TO_SERVER'):
    ...
    print(
        f"Federated Learning NUM_USERS configured to: {config.NUM_USERS} (can be dynamic for API calls)")
